...
- Explore the power of generalizing, loops, and functions for scaling-up analyses 
...
8) Analyzig multiple time periods
...
a.	What is the Maximum Contaminant Level (MCL) for nitrate (reported as nitrate-nitrogen) in mg/L.

```{r}
EPA_limit <- 
```

b.	What are the potential health impacts of consuming water with concentrations above this limit?
...
a.	Open the U.S. Geological Survey’s “Water Quality Watch” web page: <http://waterwatch.usgs.gov/wqwatch/>
b.	Under the “State” menu, select “Iowa” and allow the map to refresh.
c.	Under the measurement menu, select “Nitrate.”
d.	Move your mouse over the available triangles until you find “Cedar River at Blairs Ferry Rd, at Palo, IA.” This triangle is equidistant from the top (North) and the bottom (South) of the map, and about an inch to the left of the right (East) boundary of the state. 
e.	Once you find it, click once on the triangle, and the click again on the eight-digit hydrologic unit code (HUC) number “05464420” which identifies this site.
f.	Now you are in the official USGS data site for the Cedar River at Blairs Ferry Rd, Palo, IA. Scroll down and explore the plots of the available data. We would like to see whether nitrate concentrations change over the course of the agricultural season.  
g.	Scroll back up to the box labelled “Available Parameters.” 
h.	Uncheck all except for "NO3+NO2,water,insitu"
i.	Change the starting date to April 1, 2014 and the end date to September 1, 2014. Enter these dates in YEAR-MO-DY format (i.e. 2014-04-01, and 2014-09-01, respectively). 
j.	Under the heading “Output Format,” select the “Tab-separated” circle and click “GO.”
k.	Copy the data into a text editor and save as `site_05464420.txt` into your data directory for this module.
...
```{r}
stream_data <- read_csv("data/site_05464420.txt")
```
...
Try a more general function (`read_delim()`) for import files that are not comman delimitated.  "\t" for the delim tells the function to look for a tab to separate the cells.

```{r}
stream_data <- read_delim("data/site_05464420.txt", delim = "\t")
```

Look at the imported data.  Did it work?  Look at the file again in a text editor.  What is line number has the header row?  Is there a symbol that designates a comment in the file?  

Now use the comment option in the read_delim.   You will need to add the comment symbol below in place of [INSERT SYMBOL]

```{r}
stream_data <- read_delim("data/site_05464420.txt", delim = "\t", comment = "[INSERT SYMBOL]")
...
2) A way that works with tidyverse and pipes.  `Slice` is a function that select particular rows.
...
Calculate the percentage of time that the stream exceeds.  Hints:  you will use the `ifelse()`, `sum()`, `n()`, `mutate()`, and `summerize()` functions.  Be sure you print out your results so that it shows up in the knitted document. 
...
Now we want to calculate the probablity of exceeding the EPA limit for all gauged streams and river in Iowa.  It would be very slow to manual click and download files for all guages and time points that we are interested in.  Fortunately there is a way around this that is more advanced (but hey you are studying environmental informatics).  The process of automating the download and calculations requires a few new concepts.  
...
Copy and paste the following command into a web brouser.  
...
See how this produces the same file as what you downloaded manual.
...
Create a variable called site and set it equal to our focal site number.  Similarly, create variables for the begin and end date
...
```{r}
url <- paste0("https://nwis.waterdata.usgs.gov/usa/nwis/uv/?cb_99133=on&format=rdb&site_no=", site, "&period=&begin_date=", begin_date, "&end_date=", end_date)
url
```
...
```{r}
site <- c("05412500","05418400","05418720","05420500","05455100","05465500","05464420",
        "05481000","05482000", "05482300", "05482500", "05483600", "05484000", "05484500",
        "06808500", "06817000")
```
...
Now we can use the subsetted site vector to modify the site value in the url.  See that we replaced site with site[i]

```{r}
for(i in 1:length(site)){
  url <- paste0("https://nwis.waterdata.usgs.gov/usa/nwis/uv/?cb_99133=on&format=rdb&site_no=", site[i], "&period=&begin_date=", begin_date, "&end_date=", end_date)
  print(url)
}
```

Third, combine your code from above that you used to calculate the probability of exceeding the EPA threshold, the API example, and the "for loop"" to calcuate the proability of exceeding the EPA threshold for all the streams and rivers in Iowa.

```{r}
#Create a vector to hold the calculation for each gauge
#rep() is a function that repeats NA for a specified number of times length(site)
site <- c("05412500","05418400","05418720","05420500","05455100","05465500","05464420",
        "05481000","05482000", "05482300", "05482500", "05483600", "05484000", "05484500",
        "06808500", "06817000")
exceed <- rep(NA, length(site))
#This tibble is where we will save the calculated probability of exceeding the limit for each site.  The first column is the site number and the second is where you will add your calculation
site_exceed <- tibble(site = site, exceed = exceed)
EPA_limit <-

for(i in 1:length(site)){
  
    download.file(url = paste0("https://nwis.waterdata.usgs.gov/usa/nwis/uv/?cb_99133=on&format=rdb&site_no=", site[i], "&period=&begin_date=", begin_date, "&end_date=", end_date),
                dest = paste0("data/", site[i], ".txt"))

  stream_data <- read_delim(paste0("data/", site[i], ".txt"), delim = "\t", comment = "#")
  if(ncol(stream_data) > 5){ #THIS IS NEED TO SKIP OVER POTENTIALLY BAD DATA FILES
    ...
    site_exceed$exceed[i] <- #YOUR CALCULATION FOR THE SITE
  }else{
    site_exceed$exceed[i] <- NA
  }
...
**Question 3**
Answer the following question by reusing and modifying the code above:  What was the probability of exceeding the EPA threshold for all gauges in Virginia between 2014-04-01 and 2014-09-01?

You will get the list of Virginia gauges by 
1) Going to https://waterwatch.usgs.gov/
...
**Question 4:**
How does the probablity of exceeding the EPA threshold differ between Iowa and Virginia?  How might this be related to differences in land-use between the states?
...
**Optional Question 6:**
Is the probablity of exceeding the EPA threshold changing over time?
...
This module was initially developed by Castendyk, D. and Gibson, C. 30 June 2015. Project EDDIE: Water Quality. Project EDDIE Module 6, Version 1. http://cemast.illinoisstate.edu/data-for-students/modules/water-quality.shtml. Module development was supported by NSF DEB 1245707.